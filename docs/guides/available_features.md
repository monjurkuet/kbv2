# KBV2 Available Features

This guide describes features that are **built but not actively used** in the main ingestion pipeline. These features can be integrated or used independently.

---

## Reranking Pipeline

**Purpose:** Improve search results with multi-stage ranking.

### What It Does

1. Runs hybrid search (vector + BM25) to get initial candidates
2. Reranks results using cross-encoder model
3. Optionally fuses multiple query results with RRF (Reciprocal Rank Fusion)

### When to Use

- Searching the knowledge base (not during ingestion)
- When you want more accurate search results
- For multi-query searches needing result fusion
